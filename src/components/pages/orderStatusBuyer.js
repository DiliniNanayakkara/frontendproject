...
export default function approvedOrdersView() {
  ...
  useEffect(() => {
    axios.get("http://localhost:5000/approvedOrdersView").then((response) => {
        ...
        {artworkList.map((val) => {
          return (
            <div className="Arts">
              <div className="img">
                {" "}
                <img src="https://www.creativeboom.com/uploads/articles/a4/a40eeac492a143d5bb34412dfe8f275a8834e41c_810.jpg"></img>
              </div>
              <div className="name"> {val.artwork_name}</div>
              <div className="des">
                {" "}
                {val.artwork_category} , {val.artwork_description}
              </div>
              <div className="price">Rs. {val.artwork_price}.00</div>
              <Link to="/artworkdetail" onClick={closeMobileMenu}>
